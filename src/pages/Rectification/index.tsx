...
export function RectificationPage() {
  ...
  const [viewMode, setViewMode] = useState<'deadline' | 'school'>('deadline');
  ...
  const groupedBySchool = rectifications.reduce<
    Record<string, { schoolName: string; items: Rectification[] }>
  >((acc, r) => {
    if (!acc[r.schoolId]) {
      acc[r.schoolId] = { schoolName: r.schoolName, items: [] };
    }
    acc[r.schoolId].items.push(r);
    return acc;
  }, {});

  const RectificationCard = ({ rectification }: { rectification: Rectification }) => {
